...
type Draft = Partial<Product> & {
  ...
  stock?: number | null;
  currency?: string;
  sub_category?: "product" | "food" | "other";
};
...
function ProductForm({
  ...
  function onPickFromGallery() { fileInputRef.current?.click(); }
  function onOpenCamera() { cameraInputRef.current?.click(); }
  function removeAt(i: number) { const arr = [...files]; arr.splice(i, 1); setFiles(arr); }
  function clearFiles() { setFiles([]); }
  function previewURL(f: File) { return URL.createObjectURL(f); }

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setSubmitting(true);
    try { await onSubmit(draft, files, replaceImages); }
      ...
      <div className="row g-2">
          ...
          <div className="row g-2">
            ...
            <div className="col-4">
              ...
              <select
                ...
                onChange={(e) => setDraft((d) => ({ ...d, sub_category: e.target.value as Draft["sub_category"] }))}
              >
                <option value="product">product (African Market)</option>
          ...
          <div className="row g-2 mt-1">
            <div className="col-6">
              <div className="form-check mt-4">
                ...
                />
                <label htmlFor="feat" className="form-check-label">Mis en avant</label>
              </div>
            </div>
            <div className="col-6">
              <div className="form-check mt-4">
                <input
                  ...
                  onChange={(e) => setDraft((d) => ({ ...d, promo_eligible: e.target.checked ? 1 : 0 }))}
                />
                <label htmlFor="promo" className="form-check-label">Éligible promo</label>
              </div>
            </div>
          </div>
        ...
        <div className="col-12 col-md-4">
          ...
          <div className="d-flex flex-wrap gap-2 mb-2">
            <button type="button" className="btn btn-outline-dark btn-sm" onClick={onPickFromGallery}>
              Depuis la galerie
            </button>
            <button type="button" className="btn btn-dark btn-sm" onClick={onOpenCamera}>
              Ouvrir la caméra
            </button>
            <button type="button" className="btn btn-outline-secondary btn-sm" onClick={clearFiles} disabled={!files.length}>
              Vider
            </button>
          </div>

          <input ref={fileInputRef} type="file" accept="image/*" multiple hidden onChange={(e) => addFiles(e.target.files)} />
          <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" hidden onChange={(e) => addFiles(e.target.files)} />
          ...
          <div className="form-check mb-2">
            ...
            />
            <label htmlFor="replace_images" className="form-check-label">Remplacer la galerie existante</label>
          </div>
...
export default function ProductsAdminPage() {
  ...
  function openCreate() { setEdit(null); setShowForm(true); setOk(null); setError(null); }
  ...
  function closeForm() { setShowForm(false); setEdit(null); }
  ...
  async function onSave(draft: Draft, files: File[], replaceImages: boolean) {
    ...
    try {
      if (edit == null) {
        await createProduct(draft, files);
        setOk("Produit créé avec succès.");
      } else {
        await updateProduct(edit.id, draft, files, replaceImages);
  ...
  function resetSearch() { setQ(""); setPage(1); }
    ...
    <div className="container-xxl py-4">
      <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-2 mb-3">
        ...
        <div className="d-flex gap-2">
          <button className="btn btn-outline-secondary" onClick={refresh} disabled={loading || busy}>Actualiser</button>
          <button className="btn btn-dark" onClick={openCreate} disabled={busy}>+ Nouveau produit</button>
        </div>
      </div>
      ...
      <div className="d-flex align-items-center justify-content-between mb-3">
        <div className="input-group" style={{ maxWidth: 420 }}>
          ...
          />
          <button className="btn btn-outline-secondary" onClick={resetSearch} disabled={!q}>Effacer</button>
        </div>

        <div className="btn-group">
          <button className="btn btn-sm btn-outline-dark" disabled={page <= 1 || busy} onClick={() => setPage((p) => p - 1)}>◀</button>
          <span className="btn btn-sm btn-outline-dark disabled">{page} / {pages}</span>
          <button className="btn btn-sm btn-outline-dark" disabled={page >= pages || busy} onClick={() => setPage((p) => p + 1)}>▶</button>
        </div>
      </div>
        ...
        <div className="card-body">
                  ...
                  {filtered.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td className="text-end">
                        <div className="btn-group">
                          <button className="btn btn-sm btn-outline-dark" onClick={() => openEdit(p.id)} disabled={busy}>Modifier</button>
                          <button className="btn btn-sm btn-outline-danger" onClick={() => onDelete(p.id)} disabled={busy}>Supprimer</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          {!loading && filtered.length > 0 ? (
            <div className="d-flex justify-content-between align-items-center mt-2">
              ...
              <div className="btn-group">
                <button className="btn btn-sm btn-outline-dark" disabled={page <= 1 || busy} onClick={() => setPage((p) => p - 1)}>Préc.</button>
                <span className="btn btn-sm btn-outline-dark disabled">{page} / {pages}</span>
                <button className="btn btn-sm btn-outline-dark" disabled={page >= pages || busy} onClick={() => setPage((p) => p + 1)}>Suiv.</button>
              </div>
            </div>
          ) : null}
        </div>
          ...
          <div className="modal-dialog modal-lg modal-dialog-scrollable">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <ProductForm initial={edit || undefined} onSubmit={onSave} onCancel={closeForm} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="row g-3">
                  <div className="col-12 col-md-6">
                    {/* Image principale = 1ère image ou cover */}
                  ...
                  <div className="col-12 col-md-6">
                    <ul className="list-unstyled mb-2">
                      ...
                      <li><strong>Boutique :</strong> {preview.shop_id}</li>
                      <li><strong>Prix :</strong> {moneyMAD(preview.price)}</li>
                      <li><strong>Stock :</strong> {(preview as any).stock ?? 0}</li>
                      <li><strong>Sous-cat :</strong> {preview.sub_category || "-"}</li>
                    </ul>
              ...
              <div className="modal-footer">
                <button className="btn btn-outline-secondary" onClick={() => setPreview(null)}>Fermer</button>
                <button className="btn btn-dark" onClick={() => { const id = preview.id; setPreview(null); openEdit(id); }}>
                  Modifier ce produit
                </button>
              </div>
            </div>
