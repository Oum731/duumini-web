...
export default function AiToolsAdminPage() {
  ...
  const metaPretty = useMemo(
    () => (metaPreview ? safeJson(metaPreview) : ""),
    [metaPreview]
  );
  const googlePretty = useMemo(
    () => (googlePreview ? safeJson(googlePreview) : ""),
    [googlePreview]
  );
  ...
  async function onPublishMeta() {
    ...
    try {
      ...
      alert(
        `Meta OK: Ad ${res.ad?.id || "—"} (${
          res.ad?.status || (res.activated ? "ACTIVE" : "PAUSED")
        })`
      );
      setMetaDraftId("");
    } catch (e: any) {
        ...
        <div className="col-12 col-lg-6">
          <div className="card border-0 shadow-sm">
            <div className="card-body">
              ...
              <div className="text-muted small">
                Aperçu campagne + bouton Publier.
              </div>
